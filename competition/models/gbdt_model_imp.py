...
class GbdtModelImp(ModelInter):
    ...
    def reg_rank_predict(self, set_obj, all=False):
        ...
        if all == False:
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain_base, self.param['num_round'], set_obj.watchlist, feval=evalerror_regrank_valid)
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain, set_obj.param['num_round'], utils.watchlist, feval=evalerror_regrank_test)
    ...
    def soft_max_predict(self, set_obj, all=False):
        ...
        if all == False:
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain_base, self.param['num_round'], set_obj.watchlist, feval=evalerror_softmax_valid)
            # (6688, 4)
            pred = bst.predict(set_obj.dvalid_base)
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain, set_obj.param['num_round'], set_obj.watchlist, feval=evalerror_softmax_test)
            pred = bst.predict(set_obj.dtest)
            w = np.asarray(range(1, model_param_conf.num_of_class + 1))
            pred = pred * w[np.newaxis, :]
            pred = np.sum(pred, axis=1)
    ...
    def soft_softkappa_predict(self, set_obj, all=False):
        ...
        if all == False:
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain_base, self.param['num_round'], set_obj.watchlist, obj=obj, feval=evalerror_softkappa_valid)
            ...
            obj = lambda preds, dtrain: utils.softkappaObj(preds, set_obj.dtrain, hess_scale=set_obj.param['hess_scale'])
            bst = xgb.train(set_obj.param, set_obj.dtrain, set_obj.param['num_round'], set_obj.watchlist, obj=obj, feval=evalerror_softkappa_test)
    ...
    def ebc_predict(self, set_obj, all=False):
        ...
        if all == False:
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain_base, self.param['num_round'], set_obj.watchlist, obj=obj, feval=evalerror_ebc_valid)
            ...
            obj = lambda preds, dtrain: utils.ebcObj(preds, dtrain)
            bst = xgb.train(set_obj.param, set_obj.dtrain, set_obj.param['num_round'], set_obj.watchlist, obj=obj, feval=evalerror_ebc_test)
    ...
    def cocr_predict(self, set_obj, all=False):
        ...
        if all == False:
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain_base, set_obj.param['num_round'], set_obj.watchlist, obj=obj, feval=evalerror_cocr_valid)
            pred = bst.predict(set_obj.dvalid_base)
            ...
            bst = xgb.train(set_obj.param, set_obj.dtrain, set_obj.param['num_round'], set_obj.watchlist, obj=obj, feval=evalerror_cocr_test)
            pred = bst.predict(set_obj.dtest)
            pred = utils.applyCOCRRule(pred)
        return pred
