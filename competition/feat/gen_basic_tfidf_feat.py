...
from competition.feat.utils.feat_utils import get_sample_indices_by_relevance, dump_feat_name
...
def extract_bow_tfidf_cosine_sim_stats_feat(path, dfTrain, dfTest, feat_name, column_name, X_train, X_test, vec_type, mode, relevance_indices_dict, query_relevance_indices_dict):
    """
    ...
    if column_name in ["product_title", "product_description"]:
        ...
        ## train
        cosine_sim_stats_feat_by_relevance_train = generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values, X_train, dfTrain["id"].values, relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_train = generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values, X_train, dfTrain["id"].values, query_relevance_indices_dict, dfTrain["qid"].values)
        ...
        ## test
        cosine_sim_stats_feat_by_relevance_test = generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values, X_test, dfTest["id"].values, relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_test = generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values, X_test, dfTest["id"].values, query_relevance_indices_dict, dfTest["qid"].values)
...
def extract_svd_cosine_sim_stats_feat(path, dfTrain, dfTest, feat_name, column_name, X_svd_train, X_svd_test, mode, n_components, relevance_indices_dict, query_relevance_indices_dict):
    """
    svd sim stats feat
    :param path:
    :param feat_name:
    ...
    if column_name in ["product_title", "product_description"]:
        ...
        ## train
        cosine_sim_stats_feat_by_relevance_train = generate_dist_stats_feat("cosine", X_svd_train, dfTrain["id"].values, X_svd_train, dfTrain["id"].values, relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_train = generate_dist_stats_feat("cosine", X_svd_train, dfTrain["id"].values, X_svd_train, dfTrain["id"].values, query_relevance_indices_dict, dfTrain["qid"].values)
        ...
        with open("%s/train.%s_common_svd%d_cosine_sim_stats_feat_by_query_relevance.feat.pkl" % (path, feat_name, n_components), "wb") as f:
            cPickle.dump(cosine_sim_stats_feat_by_query_relevance_train, f, -1)
        ## test
        cosine_sim_stats_feat_by_relevance_test = generate_dist_stats_feat("cosine", X_svd_train, dfTrain["id"].values, X_svd_test, dfTest["id"].values, relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_test = generate_dist_stats_feat("cosine", X_svd_train, dfTrain["id"].values, X_svd_test, dfTest["id"].values, query_relevance_indices_dict, dfTest["qid"].values)
...
def extract_svd_cosine_sim_stats_feat_individual(path, dfTrain, dfTest, feat_name, column_name, X_svd_train, X_svd_test, vec_type, mode, n_components, relevance_indices_dict, query_relevance_indices_dict):
    """
...
def gen_bow_tfidf_by_feat_column_names(path,dfTrain, dfTest, vec_type, mode, vocabulary, relevance_indices_dict, query_relevance_indices_dict, feat_names, column_names):
    ...
    for feat_name, column_name in zip(feat_names, column_names):
        ...
        if stats_feat_flag:
            feat_list = extract_bow_tfidf_cosine_sim_stats_feat(path, dfTrain, dfTest, feat_name, column_name, X_train, X_test, vec_type, mode, relevance_indices_dict, query_relevance_indices_dict)
...
def gen_common_svd_by_feat_column_names(path,dfTrain, dfTest, X_vec_all_train, n_components, vec_type, mode, relevance_indices_dict, query_relevance_indices_dict, feat_names, column_names):
    ...
    for feat_name, column_name in zip(feat_names, column_names):
        ...
        if stats_feat_flag:
            ...
            #####################################
            feat_list = extract_svd_cosine_sim_stats_feat(path, dfTrain, dfTest, feat_name, column_name, X_svd_train, X_svd_test, mode, n_components, relevance_indices_dict, query_relevance_indices_dict)
            new_feat_names.extend(feat_list)
    return new_feat_names
...
def gen_individual_svd_by_feat_column_names(path,dfTrain, dfTest, n_components, vec_type, mode, relevance_indices_dict, query_relevance_indices_dict, feat_names, column_names):
    ...
    for feat_name, column_name in zip(feat_names, column_names):
        ...
        if stats_feat_flag:
            ...
            ## bow/tfidf-svd cosine sim stats feat ##
            #########################################
            extract_svd_cosine_sim_stats_feat_individual(path, dfTrain, dfTest, feat_name, column_name, X_svd_train, X_svd_test, vec_type, mode,
...
def extract_feat(path, dfTrain, dfTest, vec_type, mode, feat_names, column_names,vocabulary_type,svd_n_components):
    ...
    # cosine sim feat
    feat_list = extract_cosine_sim_feat(path, feat_names, mode)
    new_feat_names.extend(feat_list)
    ...
    for n_components in svd_n_components:
        feat_list = gen_common_svd_by_feat_column_names(dfTrain, dfTest, X_vec_all_train, n_components, vec_type, mode, relevance_indices_dict, query_relevance_indices_dict)
        new_feat_names.extend(feat_list)
        # cosine sim feat ##
        feat_list = extract_svd_cosine_sim_feat(path, feat_names, vec_type, mode, n_components)
        new_feat_names.extend(feat_list)

        feat_list = gen_individual_svd_by_feat_column_names(dfTrain, dfTest, n_components, vec_type, mode, relevance_indices_dict, query_relevance_indices_dict)
        new_feat_names.extend(feat_list)
...
def gen_basic_tfidf_feat():
    ...
    for vec_type in vec_types:
        ...
        ## extract feat
        feat_names = extract_feat(path, df_train, df_test, "test", feat_names, column_names)
        ## dump feat name
