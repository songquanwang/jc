...
def extract_bow_tfidf_cosine_sim_stats_feat(path, feat_name, column_name, X_train, X_test, mode, relevance_indices_dict,
                                            query_relevance_indices_dict, new_feat_names):
    ...
    if column_name in ["product_title", "product_description"]:
        ...
        ## train
        cosine_sim_stats_feat_by_relevance_train = generate_dist_stats_feat("cosine", X_train,
                                                                            dfTrain["id"].values,
                                                                            X_train, dfTrain["id"].values,
                                                                            relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_train = generate_dist_stats_feat("cosine", X_train,
                                                                                  dfTrain["id"].values,
                                                                                  X_train, dfTrain["id"].values,
        ...
        with open("%s/train.%s_cosine_sim_stats_feat_by_query_relevance.feat.pkl" % (path, feat_name),
                  "wb") as f:
            cPickle.dump(cosine_sim_stats_feat_by_query_relevance_train, f, -1)
        ## test
        cosine_sim_stats_feat_by_relevance_test = generate_dist_stats_feat("cosine", X_train,
                                                                           dfTrain["id"].values,
                                                                           X_test, dfTest["id"].values,
                                                                           relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_test = generate_dist_stats_feat("cosine", X_train,
                                                                                 dfTrain["id"].values,
                                                                                 X_test, dfTest["id"].values,
                                                                                 query_relevance_indices_dict,
                                                                                 dfTest["qid"].values)
...
def extract_cosine_sim_feat(path, feat_names, mode, new_feat_names):
    ## cosine sim feat ##
    #####################
...
def extract_svd_cosine_sim_stats_feat_all(path, feat_name, column_name, X_svd_train, X_svd_test, mode, n_components,
                                          relevance_indices_dict, query_relevance_indices_dict, new_feat_names):
    if column_name in ["product_title", "product_description"]:
        print "generate common %s-svd%d stats feat for %s" % (vec_type, n_components, column_name)
        ## train
        cosine_sim_stats_feat_by_relevance_train = generate_dist_stats_feat("cosine", X_svd_train,
                                                                            dfTrain["id"].values,
                                                                            X_svd_train,
                                                                            dfTrain["id"].values,
                                                                            relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_train = generate_dist_stats_feat("cosine", X_svd_train,
                                                                                  dfTrain["id"].values,
                                                                                  X_svd_train,
                                                                                  dfTrain["id"].values,
                                                                                  query_relevance_indices_dict,
                                                                                  dfTrain["qid"].values)
        with open("%s/train.%s_common_svd%d_cosine_sim_stats_feat_by_relevance.feat.pkl" % (
                path, feat_name, n_components), "wb") as f:
            cPickle.dump(cosine_sim_stats_feat_by_relevance_train, f, -1)
        with open("%s/train.%s_common_svd%d_cosine_sim_stats_feat_by_query_relevance.feat.pkl" % (
                path, feat_name, n_components), "wb") as f:
            cPickle.dump(cosine_sim_stats_feat_by_query_relevance_train, f, -1)
        ## test
        cosine_sim_stats_feat_by_relevance_test = generate_dist_stats_feat("cosine", X_svd_train,
                                                                           dfTrain["id"].values,
                                                                           X_svd_test, dfTest["id"].values,
                                                                           relevance_indices_dict)
        cosine_sim_stats_feat_by_query_relevance_test = generate_dist_stats_feat("cosine", X_svd_train,
                                                                                 dfTrain["id"].values,
                                                                                 X_svd_test,
                                                                                 dfTest["id"].values,
                                                                                 query_relevance_indices_dict,
                                                                                 dfTest["qid"].values)
        with open("%s/%s.%s_common_svd%d_cosine_sim_stats_feat_by_relevance.feat.pkl" % (
                path, mode, feat_name, n_components), "wb") as f:
            cPickle.dump(cosine_sim_stats_feat_by_relevance_test, f, -1)
        with open("%s/%s.%s_common_svd%d_cosine_sim_stats_feat_by_query_relevance.feat.pkl" % (
                path, mode, feat_name, n_components), "wb") as f:
            cPickle.dump(cosine_sim_stats_feat_by_query_relevance_test, f, -1)
        ## update feat names
...
def extract_svd_cosine_sim_feat(path, feat_names, mode, n_components, new_feat_names):
    for i in range(len(feat_names) - 1):
        for j in range(i + 1, len(feat_names)):
        ...
        for mod in ["train", mode]:
            ...
            sim = np.asarray(map(cosine_sim, target_vec, obs_vec))[:, np.newaxis]
            ## dump feat
            with open("%s/%s.%s_%s_%s_common_svd%d_cosine_sim.feat.pkl" % (
                    path, mod, feat_names[i], feat_names[j], vec_type, n_components), "wb") as f:
                cPickle.dump(sim, f, -1)
        ## update feat names
        new_feat_names.append(
...
def extract_svd_cosine_sim_stats_feat_individual(path, feat_name, column_name, X_svd_train, X_svd_test, mode,
...
def extract_feat(path, dfTrain, dfTest, mode, feat_names, column_names):
    ...
    for feat_name, column_name in zip(feat_names, column_names):
        ...
        if stats_feat_flag:
            # 生成bow/tfidf cosine sim stats 特征
            ...
            extract_bow_tfidf_cosine_sim_stats_feat(path, feat_name, column_name, X_train, X_test, mode,
    ...
    #####################
    # 生成bcosine sim 特征
    extract_cosine_sim_feat(path, feat_names, mode)
    ...
    for n_components in svd_n_components:
        ...
        for feat_name, column_name in zip(feat_names, column_names):
            ...
            if stats_feat_flag:
                ...
                #####################################
                extract_svd_cosine_sim_stats_feat_all(path, feat_name, column_name, X_svd_train, X_svd_test, mode,
                                                      n_components, relevance_indices_dict,
                                                      query_relevance_indices_dict)

        #####################
        ## cosine sim feat ##
        #####################
        extract_svd_cosine_sim_feat(path, feat_names, mode, n_components, new_feat_names)
        ...
        for feat_name, column_name in zip(feat_names, column_names):
            ...
            if stats_feat_flag:
                ...
                ## bow/tfidf-svd cosine sim stats feat ##
                #########################################
                extract_svd_cosine_sim_stats_feat_individual(path, feat_name, column_name, X_svd_train, X_svd_test, mode,
                                                 n_components,relevance_indices_dict, query_relevance_indices_dict, new_feat_names)
    ...
    for vec_type in vec_types:
        ...
        for run in range(config.n_runs):
            ...
            for fold, (validInd, trainInd) in enumerate(skf[run]):
                ...
                dfTrain_train = dfTrain.iloc[trainInd].copy()
                dfTrain_valid = dfTrain.iloc[validInd].copy()
                ## extract feat
                extract_feat(path, dfTrain_train, dfTrain_valid, "valid", feat_names, column_names)
