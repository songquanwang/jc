...
def ensembleSelection(feat_folder, model_folder, model_list, cdf, cdf_test, subm_prefix,
                      ...
                      bagging_replacement=False, bagging_fraction=0.5, bagging_size=10, init_top_k=5,
    ...
    for model in model_list:
        ...
        for run in range(config.n_runs):
            for fold in range(config.n_folds):
                ...
                ##
                score = getScore(pred_list_valid[model_id, run, fold, :numValidMatrix[run][fold]],
                                 cdf_list_valid[run, fold, :])
                kappa_cv[run][fold] = quadratic_weighted_kappa(score,
                                                               Y_list_valid[run, fold, :numValidMatrix[run][fold]])
    ...
    for bagging_iter in range(bagging_size):
                ...
                for run in range(config.n_runs):
                    for fold in range(config.n_folds):
                        ...
                        if cnt == 0:
                            ...
                            pass
                        p_ens_list_valid_tmp[run, fold, :numValid] = (w_ens * p_ens_list_valid_tmp[run, fold,
                                                                              :numValid] + this_w * this_p_list_valid[
                                                                                                    run, fold,
                                                                                                    :numValid]) / (
                                                                     w_ens + this_w)
        ...
        while True:
            ...
            for model, _ in this_sorted_models:
                ...
                obj = lambda param: ensembleSelectionObj(param, p_ens_list_valid_tmp, 1., this_p_list_valid,
            ...
            for run in range(config.n_runs):
                for fold in range(config.n_folds):
                    numValid = numValidMatrix[run][fold]
                    p_ens_list_valid_tmp[run, fold, :numValid] = (w_ens * p_ens_list_valid_tmp[run, fold,
                                                                          :numValid] + best_weight * this_p_list_valid[
                                                                                                     run, fold,
                                                                                                     :numValid]) / (
                                                                 w_ens + best_weight)
            best_model = None
        ...
        # use cdf
        output = ensembleSelectionPrediction(model_folder, best_bagged_model_list[:(bagging_iter + 1)],
        ...
        subm_prefix, init_top_k, bagging_iter + 1, bagging_fraction, best_kappa_mean, best_kappa_std)
